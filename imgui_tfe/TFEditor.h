...
class TFEditor {
  public:
    ...
    void setOpacityScale(const float opacity) { opacityScale = opacity; }
    void setRange(owl::interval<float> r) { range = r; }
    void selectCMap(std::string cmapName);
    RGBAEditor *getRGBAEditor();

    void loadFromFile(const char *fname);
    void saveToFile(const char *fname);
    void saveTFSteps(const char *fname, float dec);
