...
int parse_options(int argc, char **argv, Renderer_options &ro, Viewer_options &vo) {
    CLI::App app{"Tet AMR Unstructured Volume Renderer"};
    ...
    app.add_flag(
        "--split_bvh", ro.split_bvh,
        "Split the BVH into a TLAS and BLAS hierarchy to reduce peak memory consumption during BVH construction");
